...
export function testMatcherCodegen(): void {
  ...
  if (!testOnly()) {
    ...
    if (testRule(
      "sum(?first..., ?last) do sum(?last, ?first...)",
      "sum(1, 2, 3)",
      true,
      "Spread at beginning"
    )) passed++; else failed++;
    ...
    if (testRule(
      "sum(?a, ?b) do calc_sum(?a, ?b) where[is_number(?a), is_number(?b)]",
      "sum(1, 2)",
      true,
    ...
    if (testRule(
      "neg(?x) do calc_neg(?x) where[is_number(?x)]",
      "neg(5)",
      true,
    ...
    if (testRule(
      "sqrt(?a) do calc_sqrt(?a) where[gte(?a, 0)]",
      "sqrt(4)",
      true,
    ...
    if (testRule(
      "log(?a) do calc_ln(?a) where[gt(?a, 0)]",
      "log(5)",
      true,
    ...
    if (testRule(
      "sum(?a, ?b) do result(?a, ?b) where[or(is_number(?a), is_number(?b))]",
      "sum(1, x)",
      true,
    ...
    if (testRule(
      "sum(?a, ?b) do result(?a, ?b) where[and(is_number(?a), is_number(?b))]",
      "sum(1, 2)",
      true,
    ...
    if (testRule(
      "sum(?a, ?b) do calc_sum(?a, ?b) where[is_number(?a), is_number(?b), gt(?a, ?b)]",
      "sum(5, 3)",
      true,
    ...
    if (testRule(
      'sum(?terms...) do mul(?f, sum(?quot...)) where[?f := gcd_factor([?terms...]), nontrivial_factor(?f), all_divisible_by([?terms...], ?f), [?quot...] := map_div([?terms...], ?f)]',
      "sum(2, 4, 6)",
      true,
    ...
    if (testRule(
      "solve(eq(sum(mul(?x, ?a), ?c), 0), solved_for(?x)) do div(neg(?c), ?a) where[is_symbol(?x)]",
      "sum(1, 2, 3)",
      false,
      "GCD factorization fails: gcd=1 is trivial"
    )) passed++; else failed++;
    ...
    if (testRule(
      "sum(?a, ?rest...) do mul(?n, ?a) where[ all_eq([?a, ?rest...], ?a), ?n := count([?a, ?rest...]) ]",
      "sum(1, 1, 1)",
      true,
    ...
    if (testRule(
      "fold(?f, ?acc, [?x, ?xs...]) do fold(?f, ?f(?acc, ?x), [?xs...])",
      "fold(sum, 0, [1])",
      true,
  ...
  do ?rhs`,
      ...
      true,
      "Match solve equation pattern - success"
    )) passed++; else failed++;
  }
  ...
  if (testRule(
    "sum(?a, ?b) do sum(?a, ?b) where[tag($, tmp, true)]",
    "sum(1, 2)",
    true,
  ...
  if (testRule(
    "sum(?a, ?b) do sum(?a, ?b) where[has_tag($, test)]",
    "sum(1, 2)",
    false,
  ...
  if (testRule(
    "sum(?a, ?b) do sum(?a, ?b) where[tag_eq($, test, true)]",
    "sum(1, 2)",
    false,
