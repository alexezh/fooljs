...
export function initRules(runtime: Runtime) {
  const rules: RuleMeta[] = [
    ...
    // Eval compute rules
    // -------------------------
    { id: "eval_sum_numbers" as RuleId, rule: "eval(sum(?a, ?b)) => calc_sum(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleEvalSum, tags: [...T.eval, ...T.compute, "sum"] },
    { id: "eval_mul_numbers" as RuleId, rule: "eval(mul(?a, ?b)) => calc_mul(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleEvalMul, tags: [...T.eval, ...T.compute, "mul"] },
    { id: "eval_div_numbers" as RuleId, rule: "eval(div(?a, ?b)) => calc_div(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleEvalDiv, tags: [...T.eval, ...T.compute, "div"] },
    { id: "eval_neg_number" as RuleId, rule: "eval(neg(?a)) => calc_neg(?a) where[is_number(?a)]", fn: ruleEvalNeg, tags: [...T.eval, ...T.compute, "neg"] },

    // Direct compute (no eval wrapper)
    { id: "calc_sum_numbers" as RuleId, rule: "sum(?a, ?b) => calc_sum(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleCalcSum, tags: [...T.compute, "sum"] },
    { id: "calc_mul_numbers" as RuleId, rule: "mul(?a, ?b) => calc_mul(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleCalcMul, tags: [...T.compute, "mul"] },
    { id: "calc_div_numbers" as RuleId, rule: "div(?a, ?b) => calc_div(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleCalcDiv, tags: [...T.compute, "div"] },
    { id: "calc_neg_number" as RuleId, rule: "neg(?a) => calc_neg(?a) where[is_number(?a)]", fn: ruleCalcNeg, tags: [...T.compute, "neg"] },

    // -------------------------
    // Transcendentals
    // -------------------------
    { id: "eval_pow_numbers" as RuleId, rule: "eval(pow(?a, ?b)) => calc_pow(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleEvalPow, tags: [...T.eval, ...T.compute, ...T.transcendental, "power"] },
    ...
    { id: "eval_exp_number" as RuleId, rule: "eval(exp(?a)) => calc_exp(?a) where[is_number(?a)]", fn: ruleEvalExp, tags: [...T.eval, ...T.compute, ...T.transcendental, "exp"] },
    ...
    { id: "calc_pow_numbers" as RuleId, rule: "pow(?a, ?b) => calc_pow(?a, ?b) where[is_number(?a), is_number(?b)]", fn: ruleCalcPow, tags: [...T.compute, ...T.transcendental, "power"] },
    { id: "calc_sqrt_nonneg" as RuleId, rule: "sqrt(?a) => calc_sqrt(?a) where[gte(?a, 0)]", fn: ruleCalcSqrt, tags: [...T.compute, ...T.transcendental, "sqrt"] },
    { id: "calc_ln_pos" as RuleId, rule: "log(?a) => calc_ln(?a) where[gt(?a, 0)]", fn: ruleCalcLn, tags: [...T.compute, ...T.transcendental, "log"] },
    { id: "calc_log_base" as RuleId, rule: "log(?a, ?b) => calc_log(?a, ?b) where[gt(?a, 0), gt(?b, 0)]", fn: ruleCalcLogBase, tags: [...T.compute, ...T.transcendental, "log"] },
    { id: "calc_exp_number" as RuleId, rule: "exp(?a) => calc_exp(?a) where[is_number(?a)]", fn: ruleCalcExp, tags: [...T.compute, ...T.transcendental, "exp"] },
